...
public class FileUtils {
    ...
    public static String getFileSuffix(String filename) {
        if (StringUtils.isBlank(filename) || filename.lastIndexOf(MPanConstants.POINT_STR) == MPanConstants.MINUS_ONE_INT) {
            return MPanConstants.EMPTY_STR;
        }
        return filename.substring(filename.lastIndexOf(MPanConstants.POINT_STR)).toLowerCase();
    }
    ...
    public static String getFileExtName(String filename) {
        ...
        return filename.substring(filename.lastIndexOf(MPanConstants.POINT_STR) + MPanConstants.ONE_INT).toLowerCase();
    }
    ...
    public static String generateStoreFileChunkRealPath(String basePath, String identifier, Integer chunkNumber) {
        return new StringBuffer(basePath)
                ...
                .append(File.separator)
                .append(UUIDUtil.getUUID())
                .append(MPanConstants.COMMON_SEPARATOR)
                .append(chunkNumber)
                .toString();
    }
    ...
    public static void writeFile2OutputStream(FileInputStream fileInputStream, OutputStream outputStream, long length) throws IOException {
        ...
        fileChannel.transferTo(MPanConstants.ZERO_LONG, length, writableByteChannel);
        outputStream.flush();
    ...
    public static void writeStream2StreamNormal(InputStream inputStream, OutputStream outputStream) throws IOException {
        ...
        int len;
        while ((len = inputStream.read(buffer)) != MPanConstants.MINUS_ONE_INT) {
            outputStream.write(buffer, MPanConstants.ZERO_INT, len);
        }
        outputStream.flush();
        inputStream.close();
        outputStream.close();
